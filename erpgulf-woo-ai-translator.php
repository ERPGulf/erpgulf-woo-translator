...
function erpgulf_gt_link_term_to_wpml(int $en_term_id, int $ar_term_id, string $taxonomy, string $lang_code): void
{
    ...
    // The term is only language-registered as of the write above, so run the
    // parent-language guard here. On created_product_cat it fires *before* this
    // link exists, finds no language, and skips — which is how 29 English terms
    // ended up parented under Arabic categories and vanished from English
    // subcategory lists and the mega-menu.
    if ($taxonomy === 'product_cat') {
        erpgulf_gt_fix_term_parent_lang((int) $en_term_id, (int) $en_tt_id);
    }
}
...
add_action('deleted_post_meta', 'erpgulf_gt_fitment_csv_sync', 10, 4);

/**
 * A product's compatibility meta changed.
 *
 * The old version regenerated the whole 160k-row vehicles.csv inline on every
 * change — from a fitment table it never updated, so the index went stale while
 * the CSV was rewritten thousands of times during a bulk sync.
 *
 * Now: mark the product dirty, refresh just its rows once at end of request,
 * and regenerate the CSV once afterwards however many products changed.
 */
function erpgulf_gt_fitment_csv_sync($meta_id, $post_id, $meta_key, $meta_value): void
{
    // Every indexed sub-field, not only the row-count key: a brand or year can
    // change without the count changing at all.
    if (strpos((string) $meta_key, 'add_compactable_details') !== 0) {
        return;
    }
    $post = get_post($post_id);
    if (!$post || $post->post_type !== 'product') {
    ...
    erpgulf_gt_mark_fitment_dirty((int) $post_id);
}

/**
 * Collect changed products; process them once, at end of request.
 */
function erpgulf_gt_mark_fitment_dirty(int $post_id): void
{
    global $erpgulf_gt_fitment_dirty;
    static $hooked = false;

    if (!is_array($erpgulf_gt_fitment_dirty)) {
        $erpgulf_gt_fitment_dirty = [];
    }
    $erpgulf_gt_fitment_dirty[$post_id] = true;

    if (!$hooked) {
        $hooked = true;
        add_action('shutdown', 'erpgulf_gt_flush_fitment_dirty', 1);
    }
}

function erpgulf_gt_flush_fitment_dirty(): void
{
    global $erpgulf_gt_fitment_dirty;
    if (empty($erpgulf_gt_fitment_dirty)) {
        return;
    }
    foreach (array_keys($erpgulf_gt_fitment_dirty) as $pid) {
        erpgulf_gt_fitment_reindex_product((int) $pid);
    }
    $erpgulf_gt_fitment_dirty = [];
...
/**
 * One CSV regeneration per burst of changes, never one per product.
 */
function erpgulf_gt_queue_csv_regen(): void
{
    if (function_exists('as_has_scheduled_action') &&
            as_has_scheduled_action('erpgulf_gt_csv_regen', [], 'erpgulf-gt')) {
        return;  // already pending — this burst is covered
    }
    if (function_exists('as_schedule_single_action')) {
        as_schedule_single_action(time() + 120, 'erpgulf_gt_csv_regen', [], 'erpgulf-gt');
        return;
    }
    if (!wp_next_scheduled('erpgulf_gt_csv_regen')) {
        wp_schedule_single_event(time() + 120, 'erpgulf_gt_csv_regen');
    }
}

add_action('erpgulf_gt_csv_regen', 'erpgulf_gt_csv_regen_logged');

function erpgulf_gt_csv_regen_logged(): void
{
    $r = erpgulf_gt_generate_vehicles_csv();
    if (empty($r['success'])) {
        error_log('erpgulf_gt_csv_regen FAILED: ' . ($r['message'] ?? 'unknown'));
    }
}
...
function erpgulf_gt_fitment_reindex_product($product_id)
{
    ...
    // Unpublished products stay out of the index.
...
add_action('created_product_cat', 'erpgulf_gt_fix_term_parent_lang', 20, 2);
add_action('edited_product_cat', 'erpgulf_gt_fix_term_parent_lang', 20, 2);

/**
 * A translated category must sit under the translated parent. The translator
 * has been creating English terms under the Arabic parent, which drops them
 * out of that language's subcategory lists and mega-menu entirely.
 */
function erpgulf_gt_fix_term_parent_lang($term_id, $tt_id): void
{
    global $wpdb;
    ...
    $lang = $wpdb->get_var($wpdb->prepare(
        "SELECT language_code FROM {$wpdb->prefix}icl_translations
          WHERE element_type = 'tax_product_cat' AND element_id = %d",
        $tt_id
    ));
    if (!$lang)
        return;

    $p_ttid = $wpdb->get_var($wpdb->prepare(
    ...
    $p_lang = $wpdb->get_var($wpdb->prepare(
        "SELECT language_code FROM {$wpdb->prefix}icl_translations
          WHERE element_type = 'tax_product_cat' AND element_id = %d",
        $p_ttid
    ));
    if (!$p_lang || $p_lang === $lang)
        return;

    $want = apply_filters('wpml_object_id', $parent, 'product_cat', false, $lang);
    if (!$want || (int) $want === $parent)
        return;

    // Direct update, not wp_update_term — avoids re-firing edited_product_cat.
    $wpdb->update(
        $wpdb->term_taxonomy,
        ['parent' => (int) $want],
